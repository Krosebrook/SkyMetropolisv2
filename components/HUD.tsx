    ...
    return (
        <div className="absolute inset-0 pointer-events-none flex flex-col justify-between p-4 z-10 font-sans">
            ...
            <header className="flex justify-between items-start pointer-events-auto">
                ...
                {aiEnabled && (
                    <div className="bg-indigo-900/90 p-4 rounded-2xl border border-indigo-500/50 shadow-lg backdrop-blur-xl w-80 transition-all" role="status" aria-live="polite">
                        <div className="flex justify-between items-center mb-2">
                            <span className="text-xs font-bold uppercase text-indigo-300 tracking-wider">AI Objective</span>
                            {state.isGeneratingGoal && <span className="w-2 h-2 bg-yellow-400 rounded-full animate-ping" aria-label="Processing"></span>}
                        </div>
                        {currentGoal ? (
                            <>
                                ...
                                <div className="flex justify-between items-center text-xs">
                                    ...
                                    {currentGoal.completed ? (
                                        <button 
                                            ...
                                            aria-label={`Claim reward of ${currentGoal.reward} dollars`}
                                        >
            ...
            <footer className="flex items-end justify-between pointer-events-auto">
                <div className="bg-gray-900/80 p-2 rounded-2xl border border-gray-600/50 backdrop-blur-xl flex gap-2 overflow-x-auto max-w-lg" role="toolbar" aria-label="Building Tools">
                    {Object.values(BUILDINGS).map((b) => {
                        const isDisabled = stats.money < b.cost && b.type !== BuildingType.None;
                ...
                <div className="w-80 h-40 bg-black/80 rounded-xl border border-gray-800 overflow-hidden flex flex-col relative" role="log" aria-label="News Feed">
