...
export const TrafficSystem = ({ grid }: { grid: Grid }) => {
  ...
  const carCount = Math.min(roadTiles.length, 25);
  ...
  useEffect(() => {
    ...
    stateRef.current = new Float32Array(carCount * 5); // x, y, tx, ty, progress
    const colors = new Float32Array(carCount * 3);

    for (let i = 0; i < carCount; i++) {
        const start = roadTiles[Math.floor(Math.random() * roadTiles.length)];
        stateRef.current[i*5] = start.x;
        stateRef.current[i*5+1] = start.y;
        stateRef.current[i*5+2] = start.x; // Target same as start initially
        stateRef.current[i*5+3] = start.y;
        stateRef.current[i*5+4] = 1; // Progress 1 to force retarget

        const c = new THREE.Color(CAR_COLORS[Math.floor(Math.random() * CAR_COLORS.length)]);
        colors[i*3] = c.r; colors[i*3+1] = c.g; colors[i*3+2] = c.b;
    }
    if(meshRef.current) meshRef.current.instanceColor = new THREE.InstancedBufferAttribute(colors, 3);
  }, [roadTiles, carCount]);
  ...
  useFrame((_, delta) => {
    ...
    for(let i=0; i<carCount; i++) {
        const idx = i*5;
        let x = stateRef.current[idx];
        let y = stateRef.current[idx+1];
        let tx = stateRef.current[idx+2];
        let ty = stateRef.current[idx+3];
        let p = stateRef.current[idx+4];
        ...
        stateRef.current[idx] = x; stateRef.current[idx+1] = y;
        stateRef.current[idx+2] = tx; stateRef.current[idx+3] = ty;
        stateRef.current[idx+4] = p;

        // Visual position
        const cx = THREE.MathUtils.lerp(x, tx, p) - offset;
        const cy = THREE.MathUtils.lerp(y, ty, p) - offset;
  ...
  return (
    <instancedMesh ref={meshRef} args={[BOX_GEO, undefined, carCount]} castShadow>
...
const Cloud = ({ pos }: { pos: [number, number, number] }) => {
    ...
    useFrame((state) => {
        if(ref.current) ref.current.position.x += 0.01;
        if(ref.current && ref.current.position.x > 20) ref.current.position.x = -20;
    });
    return (
